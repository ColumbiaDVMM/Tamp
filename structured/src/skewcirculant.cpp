...
void ComputeOnCPU(
  ...
  cerr<<"/FFT";
  for (int i =0; i<K_; i++)param_buffer[i] = weight[i] * assist[i];
  for (int i =0; i<M_*K_; i++) conv_buffer[i] = data_buffer[i] * assist[i%K_];
	  caffe_cpu_fft<complex<Dtype>>(1, K_, param_buffer, param_buffer);
  ...
  caffe_cpu_ifft<complex<Dtype>>(M_,K_, conv_buffer, conv_buffer);
  //caffe_cpu_ifft<Dtype>(M_, K_, conv_buffer, data_buffer);
  for(int i=0; i<M_; i++)
    for (int j=0; j<N_; j++)
      (top_data+i*N_)[j] = (std::real((conv_buffer+i*K_)[j]*std::conj(assist[j]))) /K_;
...
void GradOfInputOnCPU(
     ...
     for(int i=0; i<N_; i++)
      for(int j=0; j<K_; j++)
	(weight_buffer + i*K_)[j] = param_buffer[(K_+i-j)%K_] * D_[j] * (i<j ? (Dtype)-1.: (Dtype)1.);
...
void GradOfParamOnCPUFFT(
 ...
 for (int i=0; i<M_*K_; i++) diff_buffer[i] = data_buffer[i] *assist[i%K_];
caffe_cpu_fft<complex<Dtype>>(M_,K_,diff_buffer, diff_buffer);
    //caffe_cpu_fft<Dtype>(M_, K_, data_buffer, diff_buffer);

    for(int i=0; i<M_; i++)
      for(int j=0; j<K_; j++)
        (data_buffer + i*K_)[j] = this->Flip(bottom_data + i*K_, j);
  for(int i=0; i<M_*K_; i++) conv_buffer[i] = data_buffer[i]*assist[i%K_];
caffe_cpu_fft<complex<Dtype>>(M_,K_,conv_buffer,conv_buffer);
for (int i=0; i<M_*K_; i++) conv_buffer[i] = std::conj(conv_buffer[i]);  
//caffe_cpu_fft<Dtype>(M_, K_, data_buffer, conv_buffer);
...
for(int i=0; i<M_*K_; i++) data_buffer[i] = std::real(diff_buffer[i] *std::conj(assist[i%K_]));   
// caffe_cpu_ifft<Dtype>(M_, K_, conv_buffer, data_buffer);
...
void Forward(
 ...
 vector<int64> assist_shape(1, K_);
auto assist_buffer = core->allocateBuffer<complex<Dtype>>(assist_shape);
for (int i=0; i<K_; i++) assist_buffer->data()[i] = std::exp(complex<Dtype>(0, M_PI*i/K_));
  ...
  ComputeOnCPU(input.data(), output.data(), param.data(), assist_buffer->data(),
...
void Backward(
 ...
 GradOfInputOnCPU(top_diff.data(), param.data(), grad_of_input.data(), 
  ...
  auto bias_multiplier = core->allocateBuffer<Dtype>(bias_shape);
...
for (int i=0; i<K_; i++) assist_buffer->data()[i] = std::exp(complex<Dtype>(0, M_PI*i/K_));
  ...
  GradOfParamOnCPUFFT(top_diff.data(), input.data(), assist_buffer->data(), grad_of_param.data(),
  		   conv_buffer->data(), diff_buffer->data(),
  		   data_buffer->data(),
  		   bias_multiplier->data());
  cerr<<"grad buf llocated, insight grad: "<<grad_of_param.data()[0]
      <<","<<grad_of_param.data()[1]<<",...,"<<grad_of_param.data()[N_]<<endl;
