...
nvcc -ccbin=$CC -c -D__CUDACC__ -std=c++11 -Xcompiler -fPIC $INCLUDE -I $CAFFE_PATH/include -I $CAFFE_PATH/.build_release/src -I . $KERNELS

$CC -std=c++11 -shared $SRCS $OBJS -o libtampc.so -fPIC $INCLUDE -I $CAFFE_PATH/include -I $CAFFE_PATH/.build_release/src -I . $LIBRARY -MMD -MP -pthread -DCAFFE_VERSION=1.0.0-rc3 -DNDEBUG -DUSE_LEVELDB -DUSE_LMDB $LIBS
